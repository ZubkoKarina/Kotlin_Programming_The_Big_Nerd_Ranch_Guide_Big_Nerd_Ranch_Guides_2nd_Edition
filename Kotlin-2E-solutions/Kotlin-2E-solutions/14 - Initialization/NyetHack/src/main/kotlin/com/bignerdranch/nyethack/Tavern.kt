...
fun visitTavern() {
    ...
    val patrons: MutableSet<String> = firstNames
        .zip(lastNames) { firstName, lastName -> "$firstName $lastName" }
        .toMutableSet()
...
private fun getFavoriteMenuItems(patron: String): List<String> {
    return when (patron) {
        "Alex Ironfoot" -> menuItems.filter { menuItemTypes[it]?.contains("dessert") == true }
...
private fun placeOrder(
    ...
    if (itemPrice <= patronGold.getOrDefault(patronName, 0.0)) {
        ...
        narrate("$patronName pays $TAVERN_MASTER $itemPrice gold pieces")
...
private fun displayPatronBalances(patronGold: Map<String, Double>) {
    narrate("${player.name} intuitively knows how much money the patrons have on them")
    patronGold.forEach { (patron, balance) ->
        narrate("$patron has ${"%.2f".format(balance)} gold pieces")
    }
}
